...
exports.isAuthenticated = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    ...
    if (!access_token) {
        return next(new ErrorHandler_1.default("Login to access this resource", 400));
    }
...
const authorizeRoles = (...roles) => {
    return (req, res, next) => {
        if (!roles.includes(req.user?.role || "")) {
            return next(new ErrorHandler_1.default(`Role: ${req.user?.role} is not allowed to access`, 403));
        }
        next();
    };
};
